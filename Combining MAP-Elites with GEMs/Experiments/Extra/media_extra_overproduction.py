...
def performance(x, model, objective, m, m_original): 
    ...
    Performance = np.zeros(m.shape[1]+1)
...
def mapelites_heatmap(performance_map,  values):
    ...
    for i in range(0, performance_map.shape[0]):

        for j in range(0, performance_map.shape[1]):

            if performance_map[i,j] == 0:
                
                performance_map[i,j] = 1

    #x_axis_labels = ["sugar", "carbohydrate", "nucleotide", "lipid", "amino acid"]

    s = sns.heatmap(performance_map, linewidth=0.5, cmap = "viridis", cbar_kws={'label': 'Performance'}, norm=LogNorm())#, xticklabels = x_axis_labels,  annot = values)
...
objective_ME = "EX_etoh_e" # SUCCt3 is the final reaction to get extracellular succinate: succ_c + h_e ⇌ h_c + succ_e
...
for i in range(0, 7):
    ...
    p = performance(MAP_solutions1[1][i], model1, "SUCCt3", m1, m_original)

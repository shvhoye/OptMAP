# Combining evolutionary algorithms with genome scale modeling for intelligent metabolic engineering - Pyruvate overproduction
...
def performance(x, model, objective): 
    ...
    Performance =  -sol.fluxes["BIOMASS_Ecoli_core_w_GAM"]*sol.fluxes[objective] 
...
def ven_genes(performance_map, solution_map, model):
    ...
    for k in range(0, 3):
        ...
        #print(sol_test.fluxes["BIOMASS_Ecoli_core_w_GAM"]*sol_test.fluxes["PYRt2"])
...
def PhPP(original_model, solution, objective_ME):
    ...
    plt.plot(prod_env.flux_maximum,prod_env.PYRt2, color = 'blue')

    plt.fill_between(prod_env.flux_maximum, prod_env.PYRt2, np.min(prod_env.PYRt2), facecolor='blue', alpha=0.1)
    ...
    plt.plot(prod_env.flux_maximum,prod_env.PYRt2, color = 'green')

    plt.fill_between(prod_env.flux_maximum, prod_env.PYRt2, np.min(prod_env.PYRt2), facecolor='green', alpha=0.1)

    plt.ylabel('PYRt2 [mmol gDW^-1 h^-1]')
...
## Pyruvate overproduction

# MAP-Elites combined with the core E. coli GEM to maximize pyruvate production.
...
print(sol1.fluxes["EX_pyr_e"])

print(sol1.fluxes["PYRt2"])

print(sol1.fluxes["BIOMASS_Ecoli_core_w_GAM"]*sol1.fluxes["PYRt2"])

# 2. Run MAP-Elites 

I1 = 0#20000 # Number of generations

MAP_Elites_model = core_e_coli_model.copy()

objective_ME = "PYRt2" # PYRt2 is the final reaction to get extracellular pyruvate: pyr_c + h_e ⇌ h_c + pyr_e

MAP_solutions1, MAP_performances1, best_elite_per_generation = MAP_Elites(I1, MAP_Elites_model, objective_ME)

# np.save("Combining ME with GEMs/Results and Figures/MAP_solutions_pyr20000_v2", MAP_solutions1)

# np.save("Combining ME with GEMs/Results and Figures/MAP_performances_pyr20000_v2", MAP_performances1)

# np.save("Combining ME with GEMs/Results and Figures/best_elite_per_generation_pyr20000_v2", best_elite_per_generation)

# succ_pef = np.load("Combining ME with GEMs/Results and Figures/pyr25000/MAP_performances25000.npy")
...
print(sol_best_elite.fluxes["PYRt2"]) 

print(sol_best_elite.fluxes["BIOMASS_Ecoli_core_w_GAM"])

print(sol_best_elite.fluxes["BIOMASS_Ecoli_core_w_GAM"]*sol_best_elite.fluxes["PYRt2"]) # = the performance
...
result = optgene.run(target=model1.reactions.PYRt2,
                     biomass=model1.reactions.BIOMASS_Ecoli_core_w_GAM,
                     substrate=core_e_coli_model.metabolites.glc__D_e,
                     max_evaluations=5000,
                     plot=True)

core_e_coli_model.metabolites.glc__D_e

df = result.data_frame

df.to_csv (r'/Users/Shaun/Documents/School/2021 - 2022/Sem 2/a. Master thesis/Programming/Master Thesis/Combining ME with GEMs/Results and Figures/OptGene_results_PYRt2.csv', index = False, header=True)
...
data2.to_csv (r'/Users/Shaun/Documents/School/2021 - 2022/Sem 2/a. Master thesis/Programming/Master Thesis/Combining ME with GEMs/Results and Figures/OptMAP_results_PYRt2.csv', index = False, header=True)

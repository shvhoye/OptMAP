...
# python plot increase font size
...
def niche(x):
    ...
    sugar = np.load("Combining ME with GEMs/Supporting scripts and data/data/sugar_iML1515.npy")

    carbohydrate = np.load("Combining ME with GEMs/Supporting scripts and data/data/carbohydrate_iML1515.npy")

    nucleotide = np.load("Combining ME with GEMs/Supporting scripts and data/data/nucleotide_iML1515.npy")

    lipid = np.load("Combining ME with GEMs/Supporting scripts and data/data/lipid_iML1515.npy")

    AA = np.load("Combining ME with GEMs/Supporting scripts and data/data/AA_iML1515.npy")
...
def PhPP(original_model, solution, objective_ME):
    ...
    core_model = iML1515.copy()

    prod_env = production_envelope(core_model, objective_ME)

    plt.plot(prod_env.flux_maximum,prod_env.EX_succ_e, color = 'blue')

    plt.fill_between(prod_env.flux_maximum, prod_env.EX_succ_e, np.min(prod_env.EX_succ_e), facecolor='blue', alpha=0.1)
    ...
    plt.plot(prod_env.flux_maximum,prod_env.EX_succ_e, color = 'green')

    plt.fill_between(prod_env.flux_maximum, prod_env.EX_succ_e, np.min(prod_env.EX_succ_e), facecolor='green', alpha=0.1)

    plt.ylabel('EX_succ_e [mmol gDW^-1 h^-1]')
...
model1.objective = "EX_succ_e" #"BIOMASS_Ec_iML1515_core_75p37M"
...
print(sol1.fluxes["BIOMASS_Ec_iML1515_core_75p37M"]*sol1.fluxes["EX_succ_e"])

# 2. Run MAP-Elites 

I1 = 20000 # Number of generations

MAP_Elites_model = iML1515.copy()

objective_ME = "EX_succ_e" 

MAP_solutions1, MAP_performances1, best_elite_per_generation = MAP_Elites(I1, MAP_Elites_model, objective_ME)

# np.save("Combining ME with GEMs/Results and Figures/MAP_solutions_succ20000", MAP_solutions1)

# np.save("Combining ME with GEMs/Results and Figures/MAP_performances_succ20000", MAP_performances1)

# np.save("Combining ME with GEMs/Results and Figures/best_elite_per_generation_succ20000", best_elite_per_generation)

# succ_pef = np.load("Combining ME with GEMs/Results and Figures/MAP_performances25000.npy")

# succ_sol = np.load("Combining ME with GEMs/Results and Figures/MAP_solutions25000.npy")
...
print(sol_best_elite.fluxes["EX_succ_e"]) 

print(sol_best_elite.fluxes["BIOMASS_Ec_iML1515_core_75p37M"])

print(sol_best_elite.fluxes["BIOMASS_Ec_iML1515_core_75p37M"]*sol_best_elite.fluxes["EX_succ_e"]) # = the performance

print(sol_best_elite.fluxes) # All the fluxes

# Phenotypic phase plane (flux) = Product envelope

best_elite = np.zeros(len(model1.genes))
...
optgene = OptGene(iML1515.copy())

result = optgene.run(target=iML1515.reactions.EX_ac_e,
                     biomass=iML1515.reactions.BIOMASS_Ec_iML1515_core_75p37M,
                     substrate=iML1515.metabolites.glc__D_e,

...
def performance(x, model, objective): 
    ...
    temp_model.objective = "BIOMASS_Ecoli" 
    
    sol = temp_model.optimize()

    Performance =  sol.fluxes["BIOMASS_Ecoli"]*sol.fluxes[objective]
...
def performance2(x, model, objective): 
    ...
    biomass = sol.fluxes["BIOMASS_Ecoli"]

    Performance =  sol.fluxes["BIOMASS_Ecoli"]*sol.fluxes[objective]/-sol.fluxes["EX_glc__D_e"]
...
def niche(x):
    ...
    # sugar = np.load("Combining ME with GEMs/Supporting scripts and data/data/niches_iJR904/sugar_iJR904.npy")

    # carbohydrate = np.load("Combining ME with GEMs/Supporting scripts and data/data/niches_iJR904/carbohydrate_iJR904.npy")

    # nucleotide = np.load("Combining ME with GEMs/Supporting scripts and data/data/niches_iJR904/nucleotide_iJR904.npy")

    # lipid = np.load("Combining ME with GEMs/Supporting scripts and data/data/niches_iJR904/lipid_iJR904.npy")

    # AA = np.load("Combining ME with GEMs/Supporting scripts and data/data/niches_iJR904/AA_iJR904.npy")
...
def MAP_Elites(I, G, model, objective):
   ...
   # 1. Initialize the algorithm by randomly generating G genomes and determining the performance and features of each solution + place them in the archive
    ...
    for i in range(0, G, 1): 
        ...
        m = np.random.choice([1, 2, 3, 4]) # up to 10 mutations

        #m = np.random.choice([0, 1]) # Only wildtypes (no mutations) and organisms with 1 mutation

        #m = random.randrange(start = 0, stop = 10 , step = 1) 
...
def ven_genes(performance_map, solution_map, model):
    ...
    for k in range(0, 3):
        ...
        #print(sol_test.fluxes["BIOMASS_Ecoli"]*sol_test.fluxes["SUCCt3"])
...
def PhPP(original_model, solution, objective_ME):
    ...
    plt.ylabel('NARGt [mmol gDW^-1 h^-1]')
    plt.xlabel('BIOMASS_Ecoli [h^-1]')
...
iJR904 = cobra.io.read_sbml_model("Combining ME with GEMs/Models/iJR904.xml")

len(iJR904.metabolites) # 761
len(iJR904.reactions) # 1075
len(iJR904.genes) # 904

# 1. FBA of the original iJR904 model

model1 = iJR904.copy()

model1.objective = "BIOMASS_Ecoli"

sol1 = model1.optimize()

print(sol1.fluxes["BIOMASS_Ecoli"])
...
atp_c = iJR904.metabolites.get_by_id("atp_c")
coa_c = iJR904.metabolites.get_by_id("coa_c")
amp_c = iJR904.metabolites.get_by_id("amp_c")
ppi_c = iJR904.metabolites.get_by_id("ppi_c")
malcoa_c = iJR904.metabolites.get_by_id("malcoa_c")
co2_c = iJR904.metabolites.get_by_id("co2_c")
...
len(e_coli_flavanone.metabolites) # 761 ->  767
len(e_coli_flavanone.reactions) # 1075 -> 1083
len(e_coli_flavanone.genes) # 904 -> 909
...
model2.objective = "BIOMASS_Ecoli"

sol2 = model2.optimize()

print(sol2.fluxes["BIOMASS_Ecoli"])
...
model1 = e_coli_flavanone.copy()

# PhPP NARGt

core_model = e_coli_flavanone.copy()

prod_env = production_envelope(core_model,  "NARGt" )
...
I1 = 0#1000 # Number of generations
...
MAP_solutions1, MAP_performances1, best_elite_per_generation = MAP_Elites(I1, G1, MAP_Elites_model, objective_ME)

# np.save("Combining ME with GEMs/Results and Figures/MAP_solutions_ACCOAC_5000", MAP_solutions1)

# np.save("Combining ME with GEMs/Results and Figures/MAP_performances_ACCOAC_5000", MAP_performances1)

# np.save("Combining ME with GEMs/Results and Figures/best_elite_per_generation_ACCOAC_5000", best_elite_per_generation)

# best_elite_per_generation = np.load("Combining ME with GEMs/Results and Figures/best_elite_per_generation_ACCOAC_5000.npy")

# succ_pef = np.load("Combining ME with GEMs/Results and Figures/MAP_performances_ACCOAC_5000.npy")

# succ_sol = np.load("Combining ME with GEMs/Results and Figures/MAP_solutions_ACCOAC_5000.npy")
...
print(sol_best_elite.fluxes["BIOMASS_Ecoli"])

print(sol_best_elite.fluxes["BIOMASS_Ecoli"]*sol_best_elite.fluxes["ACCOAC"]) # = the performance
...
best_elite = np.zeros(len(iJR904.genes))
...
optgene = OptGene(iJR904.copy())

result = optgene.run(target=model1.reactions.ACCOAC,
                     biomass=model1.reactions.BIOMASS_Ecoli,
...
df.to_csv (r'/Users/Shaun/Documents/School/2021 - 2022/Sem 2/a. Master thesis/Programming/Master Thesis/Combining ME with GEMs/Results and Figures/OptGene_results_accoac.csv', index = False, header=True)

result.plot(plotter, 0)

# df OptMAP

data = {'Solution': ['1'], 'reactions': [('KAS15', 'ACOATA')], 'genes': ['b1091'], 'target_flux': ['2.42'], 'biomass_flux': ['0.92'], 'fitness': ['2.23']}

data2 = pd.DataFrame(data)

data2.to_csv (r'/Users/Shaun/Documents/School/2021 - 2022/Sem 2/a. Master thesis/Programming/Master Thesis/Combining ME with GEMs/Results and Figures/OptMAP_results_accoac.csv', index = False, header=True)
...
performance2(best_elite, model1, objective_ME)


#---

def genes_map(map, model):

    for k in range(0, 5):

        for h in range(0, 3):

            #print(MAP_solutions1[h][k])
...
genes_map(MAP_solutions1, model1)
...
for k in range(0, 100):
    ...
    x = initial_mutant_v2(n, n)

    #x = initial_mutant(n, 1)

    i = np.where(x == 1)[0] 

    print(sugar[i], carbohydrate[i], nucleotide[i], lipid[i], AA[i])
